...
type ReflectionEditorProps = Pick<
  SessionReflectionFieldProps,
  "value" | "onChange" | "placeholder" | "textareaClassName"
>
...
type ReflectionEditorFallbackProps = Pick<
  SessionReflectionFieldProps,
  "placeholder" | "textareaClassName"
>

function ReflectionEditorFallback({
  placeholder,
  textareaClassName,
}: ReflectionEditorFallbackProps) {
  return (
    <div
      className={cn(
        "min-h-[250px] rounded-lg border-0 bg-muted/30 px-5 py-4 shadow-none",
        textareaClassName,
      )}
      aria-hidden
    >
      <div className="mb-2 h-8 border-b border-border/50 pb-2" />
      <p className="min-h-[190px] px-0 py-1 text-base leading-relaxed text-muted-foreground/50">
        {placeholder}
      </p>
    </div>
  )
